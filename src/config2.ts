// Config v2: an ordered list of services, first matters most. Each service is
// one plugin's kind of program, run as a systemd unit, a docker container, a
// command, or a group of those that share the card.
...
export function checkConfig(input: unknown) {
  ...
  const listenPort = Number(new URL(`http://${listen}`).port);
